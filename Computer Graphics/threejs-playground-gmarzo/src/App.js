...
const App = () => {
  ...
  const stopColorCycle = event => {
    universe.holdColor()
  }

  const moveEmerald = event => {
    console.log('DOOOOOOOOODDDDDGGGGE')
    universe.cast.donut.position.x += 0.1
  }

  // const printColor = event => {
  //   console.log(universe.cast.garnet.material.color)
  // }
    ...
    <section className="App">
      ...
      <footer>
        <section>
          ...
          <button disabled={!universe} onClick={cycleColor}>
            Cycle Color
          </button>

          <button disabled={!universe} onClick={stopColorCycle}>
            Stop Color Change
          </button>
        </section>

        <section>
          <button disabled={!universe} onClick={moveEmerald}>
            Move Ring
          </button>
