...
public class T3State {
    ...
    public Map<T3Action,T3State> getTransitions () {
        Map<T3Action, T3State> moves = new HashMap<T3Action, T3State>();
        T3Action possMove = null;
        int[] action = getMoves();
        for (int x = 0; x < 3; x++) {
            for (int y = 0; y < 3; y++) {
                for (int i = 0; i < action.length; i++) {
                        possMove = new T3Action(x, y, action[i]);
                        if (!(isValidAction(possMove))) continue;
                        moves.putIfAbsent(possMove, getNextState(possMove));
                }
