        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.beliefs:
            ...
            for new_pos, prob in newPosDist.items():
                newBeliefs[new_pos] += prob * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while particles > 0:
            for i in range(len(self.legalPositions)):
                if particles == 0:
                    break
                positions.append(self.legalPositions[i])
                particles -= 1
    ...
    def observe(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            weightedParticles[particle] += emissionModel[distance]
        ...
        for i in range(len(self.particles)):
            sampledParticle = util.sample(weightedParticles)
            self.particles[i] = sampledParticle
