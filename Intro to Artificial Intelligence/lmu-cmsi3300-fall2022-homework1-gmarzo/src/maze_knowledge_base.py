...
class MazeKnowledgeBase:
    ...
    def ask (self, query):
        ...
        query_kb = self.clauses.copy()

        #Add attc to kb
        for clause in query.props.items():
            query_kb.add(MazeClause([(clause[0], not clause[1])]))
        
        new = set()

        while True:
            for clause1 in query_kb:
                for clause2 in  query_kb:
                    if clause1 == clause2:
                        continue
            ...
            if new.issubset(query_kb):
                return False
            query_kb = query_kb.union(new)
